...
import (
	...
	"github.com/ethereum/go-ethereum/fl"
...
type Node struct {
	...
	flHttpPort     int          // HTTP post
	httpEndpoint   string       // HTTP endpoint (interface + port) to listen at (empty = HTTP disabled)
	flHttpEndpoint string       // HTTP endpoint (interface + port) to listen at (empty = HTTP disabled)
	httpWhitelist  []string     // HTTP RPC modules to allow through this endpoint
	httpCors       string       // HTTP RPC Cross-Origin Resource Sharing header
	httpListener   net.Listener // HTTP RPC listener socket to server API requests
	flHttpListener net.Listener // HTTP RPC listener socket to server API requests
	httpHandler    *rpc.Server  // HTTP RPC request handler to process the API requests
	flHttpHandler  *fl.Server   // HTTP RPC request handler to process the API requests
...
func New(conf *Config) (*Node, error) {
	...
	return &Node{
		...
		flHttpPort:     conf.FLHTTPPort,
		httpEndpoint:   conf.HTTPEndpoint(),
		flHttpEndpoint: conf.FLHTTPEndpoint(),
...
func (n *Node) startHTTP(endpoint string, apis []rpc.API, modules []string, cors string) error {
	...
	flHandler := fl.NewServer(n.EventMux())
	for _, api := range apis {
		if whitelist[api.Namespace] || (len(whitelist) == 0 && api.Public) {
			if err := handler.RegisterName(api.Namespace, api.Service); err != nil {
				return err
			}
			if err1 := flHandler.RegisterName(api.Namespace, api.Service); err1 != nil {
				return err1
			}
	...
	log.Println("====startHTTP====" + n.flHttpEndpoint)
	if flListener, err1 = net.Listen("tcp", n.flHttpEndpoint); err1 != nil {
		return err1
	}
	go rpc.NewHTTPServer(cors, handler).Serve(listener)
	glog.V(logger.Info).Infof("HTTP endpoint opened: http://%s", endpoint)
	go fl.NewHTTPServer(cors, flHandler).Serve(flListener)

	// All listeners booted successfully
	n.httpEndpoint = endpoint
	n.flHttpEndpoint = endpoint
	n.httpListener = listener
	n.flHttpListener = flListener
	n.httpHandler = handler
	n.flHttpHandler = flHandler
	n.httpCors = cors
